...
def list_recipes():
    for key, val in cookbook.items():
        print("- %s is a %s that will take you %d minutes" % (key, val['meal'], val['prep_time']))
...
def print_recipe(name):
    ...
    try:
        ...
        print("The preparation should take you %d minutes" % recipe["prep_time"])
    except KeyError:
...
def get_recipe_features():
    ...
    ingredients = input("What are the ingredients? Separated by comas. ").split(',')
...
switcher = { 1 : get_recipe_features, 2 : get_recipe_to_delete, 3 : get_recipe_to_print, 4 : list_recipes }
